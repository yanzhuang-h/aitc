...
# 待确认的重复键: "13G2O0C6CL013G2A0C6CL00" "13G2A0C6CL013G1A0C6CL00"
...
def get_Cross_id_online_state(Online_map,cross_id):
    time_scatter = dict()
    for i in range(0,24):
        hour = str(i)
        time_scatter[hour] = {'L': [0, 0, 0, 0, 0], 'R': [0, 0, 0, 0, 0],'U': [0, 0, 0, 0, 0],'D': [0, 0, 0, 0, 0]}


    return time_scatter
    
def get_flow_map(file_path):
    ...
    for line in flow:
        ...
        try:
            ...
            # print(ti)
            if ti not in Flow_map:
                n = np.zeros((60, 7))
                Flow_map[ti] = n
            road_id = int(data['jtll_ddbh'])
            if road_id>100:
                road_id = 50+road_id%100
            # print(int(data['jtll_ddbh']))
            Flow_map[ti][road_id][int(data['ycsb_cdbh'])] += 1
            # print(flow_map[ti][int(data['jtll_ddbh'])][int(data['ycsb_cdbh'])])
            # print(flow_map[ti])
        except:
            continue
    return Flow_map

def get_stage_map(file_path):
    with open(file_path, "r", encoding="utf-8") as f:
        stage = f.readlines()
    Stage_map = dict()
    for line in stage:
        ...
        try:
            data = json.loads(line)
            # print(data)
            ti = int(int(data["time"] / 1000))
            if ti not in Stage_map:
                Stage_map[ti] = dict()
            Stage_map[ti][data['CrossId']] = data['curStageLen'], data['curStageNo']
        except:
            continue
    return Stage_map
...
def get_term_ans1(stage_path,cross_id,T,num):
    stage_map = get_stage_map(stage_path)
    term = dict()
    add = [0]*10
    mark =[0]*10
    Last = 0
    for ti in stage_map:
        if cross_id in stage_map[ti]:
            # print(stage_map[ti][cross_id])
            Len = int(stage_map[ti][cross_id][0])
            NO = int(stage_map[ti][cross_id][1])
            if NO >0:
                NO = NO%10
            if  NO == T[0] and Last !=T[0]:
                # print(sum(mark))
                if sum(mark)>=num:
                    term[ti] = [0]*10
            ...
            for z in range(len(T)):
                if NO == T[z] and Last != T[z]:
                    add[z] = ti - Len
                    mark[z]=1
            Last = NO
    return term

def get_term_ans2(stage_path,cross_id,T,num):
    stage_map = get_stage_map(stage_path)
    term = dict()
    add = [0]*10
    mark =[0]*10
...
def get_road_schedule(path,road_id):
    schedule_file_path = get_path('schedule_json')
    road_path = path+road_id+".json"
    road_path = os.path.join(schedule_file_path, road_path)
    with open(road_path, "r", encoding="utf-8") as f:
        schedule = json.load(f)
        return schedule

def KongF_Check(ti,during,road_id,start,flow_map,pass_time):
    during = min(during,100)
    l = 0
    mark = [0,0,0,0,0,0,0]
    kong = [during, during,during, during, during,during,during]
    for i in range(ti+start, ti+during):
        if i in flow_map:
            # print(flow_map[i][road_id],i-ti)
            # print(mark,kong)
            for z in range(0, 7):
                if z >= 0 and z <= 5:
                    l += flow_map[i][road_id][z]
                if flow_map[i][road_id][z] == 0:
                    mark[z]+=1
                else:
                    mark[z]=0
                if mark[z]>=pass_time and kong[z]==during:
                    if i-ti >during-4:
                        kong[z] = during+6
                    else:
                        kong[z]=i-ti
