...
class AITCApplication:
    ...
    def __init__(self, *, config_sync_manager, http_server, tcp_server, decision_pipeline, prediction_scheduler, send_interval, enable_config_sync=True, enable_prediction_scheduler=True, logger=None):
...
def create_application(logger=None, settings: RuntimeSettings | None = None) -> AITCApplication:
    ...
    http_server = HttpRuntimeServer(host=settings.http_host, port=settings.http_port, ingestor=ingestor, config_service=config_service, query_service=query_service, signal_timing_tool=signal_timing_tool, qwen_agent=qwen_agent, control_process_agent=control_process_agent, logger=logger)
