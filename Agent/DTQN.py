...
class DTQN(nn.Module):
    def __init__(self, *, data_size,Len_time=600,flow_size=28,queue_size=56,stage_size=2,hidden_dim=64,T_size=1034, num_classes=1, dim=1024, depth=24, heads=16, mlp_dim=1024, dim_head = 128):
        super().__init__()
...
if __name__ == "__main__":
    ...
    for i in range(20):
        t = torch.randn(1,600, 86).to(device)
        t1 = torch.randn(1, 10).to(device)
        choose = [56, 32]
        x, p1, p2 = a(t, t1, choose)
    print(time.localtime(time.time()))
