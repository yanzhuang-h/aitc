...
- 2026-08-06: 将长期记忆中的持久化配置池也补齐为一等只读查询能力。`MemoryQueryLayer.get_config_pool()` 读取 `LongTermMemory` 中的配置项，`DataQueryTools.query_config_pool` 对 Agent 暴露配置池查询；运行时配置快照仍由 `ConfigService` 负责，避免混淆。全量自动化测试增至 50 项并通过。
- 2026-08-06: 根据 RTX 5060 Laptop 的 `sm_120` 与旧 PyTorch wheel 不兼容问题，新增 `app.infrastructure.llm.OpenAICompatibleLLMClient`，正式模型接入改为调用 vLLM/SGLang 等 OpenAI-compatible 服务；新增 LLM 环境配置、客户端契约测试和 `infra/model/qwen/README.md` 部署说明。本地 transformers 脚本仅保留为 smoke test，Qwen 权重继续忽略不提交；全量自动化测试增至 51 项并通过。
