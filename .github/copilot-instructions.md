...
## 硬性规则
> 权限说明（2026-09-24 起）：仓库内**所有文件均可修改**，包括 `lib/` 与共享文件；
> 但「可改」不等于「可以乱改」，改动仍按下面的最小改动原则执行。

1. **`lib/` 与共享文件可改，改动最小化**：默认只做包装、边界调整与冗余清理，不改变算法行为；确需修改算法逻辑时，必须在提交信息与 PR 描述中写明理由、影响面与验证方式。
...
4. **保持对外兼容**：接口、报文格式与既有行为保持兼容；重构优先只移动调用边界，不改变算法语义。
5. **减法优先**：只做"能合并才合并"的精简，用户明确要求时才抽象新层，避免过度架构化。
6. **共享文件的改动要说清影响面**：`Server_AITC.py`、`lib/` 等共享文件不再限制改动范围，但每个 PR 需说明影响面，并给出编译 + 单测（必要时回放）的验证结论。
...
lib/                  算法与算法配置层（DQN 等；可改，改动须保持行为兼容）
test/                 自动化测试 + 手工回放客户端
```
