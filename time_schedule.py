...
from lib.intersection_config import (
    delete_cross_info_config,
    delete_intersection_config,
    delete_road_info_config,
    delete_online_intersection_config,
    delete_schedule_config,
    get_cross_info_config,
    get_intersection_config,
    get_road_info_config,
    get_online_intersection_config,
    get_schedule_config,
    list_cross_info_configs,
    list_intersection_configs,
    list_road_info_configs,
    list_online_intersection_configs,
    validate_and_save_cross_info_config,
    validate_and_save_intersection_config,
    validate_and_save_road_info_config,
    validate_and_save_online_intersection_config,
    validate_and_save_schedule_config,
)
...
def _request_bool_arg(name, default):
...
@app.route('/road_info', methods=['GET'])
def list_road_info_api():
    full = _request_bool_arg("full", False)
    try:
        return jsonify(list_road_info_configs(full=full))
    except Exception as e:
        logger.error(f"鑾峰彇 road_info 閰嶇疆鍒楄〃澶辫触: {str(e)}")
        return jsonify({
            "status": "error",
            "saved": False,
            "reason": str(e)
...
@app.route('/road_info/<cross_id>', methods=['GET'])
def get_road_info_api(cross_id):
    try:
        return jsonify(get_road_info_config(cross_id))
    except Exception as e:
        logger.error(f"鑾峰彇 road_info 閰嶇疆澶辫触: {str(e)}")
        return jsonify({
            "status": "error",
            "saved": False,
            "reason": str(e)
...
@app.route('/road_info/validate', methods=['POST'])
def validate_road_info_api():
    payload = request.get_json(silent=True)
    if not isinstance(payload, dict):
        return jsonify({
            "status": "error",
    ...
    result = validate_and_save_road_info_config(payload, dry_run=True)
    return jsonify(result), 200 if result.get("status") != "error" else 400


@app.route('/road_info/update', methods=['POST'])
def update_road_info_api():
    payload = request.get_json(silent=True)
    if not isinstance(payload, dict):
        return jsonify({
            "status": "error",
    ...
    result = validate_and_save_road_info_config(payload, dry_run=False)
    return jsonify(result), 200 if result.get("status") != "error" else 400


@app.route('/road_info/delete', methods=['POST'])
def delete_road_info_api():
    payload = request.get_json(silent=True)
    if not isinstance(payload, dict):
        return jsonify({
            "status": "error",
    ...
    dry_run = bool(payload.get("dry_run", False)) or _request_bool_arg("dry_run", False)
    result = delete_road_info_config(payload, dry_run=dry_run)
    return jsonify(result), 200 if result.get("status") != "error" else 400


@app.route('/road_info/<cross_id>', methods=['DELETE'])
def delete_road_info_by_id_api(cross_id):
    payload = request.get_json(silent=True)
    if not isinstance(payload, dict):
        payload = {}
    payload["Cross_id"] = cross_id

    result = delete_road_info_config(payload, dry_run=_request_bool_arg("dry_run", False))
    return jsonify(result), 200 if result.get("status") != "error" else 400


@app.route('/cross_info_config', methods=['GET'])
def list_cross_info_config_api():
    full = _request_bool_arg("full", False)
    try:
        return jsonify(list_cross_info_configs(full=full))
    except Exception as e:
        logger.error(f"鑾峰彇 cross_info 閰嶇疆鍒楄〃澶辫触: {str(e)}")
        return jsonify({
            "status": "error",
            "saved": False,
            "reason": str(e)
...
@app.route('/cross_info_config/<cross_id>', methods=['GET'])
def get_cross_info_config_api(cross_id):
    try:
        return jsonify(get_cross_info_config(cross_id))
    except Exception as e:
        logger.error(f"鑾峰彇 cross_info 閰嶇疆澶辫触: {str(e)}")
        return jsonify({
            "status": "error",
            "saved": False,
            "reason": str(e)
...
@app.route('/cross_info_config/validate', methods=['POST'])
def validate_cross_info_config_api():
    payload = request.get_json(silent=True)
    if not isinstance(payload, dict):
        return jsonify({
            "status": "error",
    ...
    result = validate_and_save_cross_info_config(payload, dry_run=True)
    return jsonify(result), 200 if result.get("status") != "error" else 400


@app.route('/cross_info_config/update', methods=['POST'])
def update_cross_info_config_api():
    payload = request.get_json(silent=True)
    if not isinstance(payload, dict):
        return jsonify({
            "status": "error",
    ...
    result = validate_and_save_cross_info_config(payload, dry_run=False)
    return jsonify(result), 200 if result.get("status") != "error" else 400


@app.route('/cross_info_config/delete', methods=['POST'])
def delete_cross_info_config_api():
    payload = request.get_json(silent=True)
    if not isinstance(payload, dict):
        return jsonify({
            "status": "error",
    ...
    dry_run = bool(payload.get("dry_run", False)) or _request_bool_arg("dry_run", False)
    result = delete_cross_info_config(payload, dry_run=dry_run)
    return jsonify(result), 200 if result.get("status") != "error" else 400


@app.route('/cross_info_config/<cross_id>', methods=['DELETE'])
def delete_cross_info_config_by_id_api(cross_id):
    payload = request.get_json(silent=True)
    if not isinstance(payload, dict):
        payload = {}
    payload["Cross_id"] = cross_id
    payload["delete_code"] = _request_bool_arg("delete_code", payload.get("delete_code", True))

    result = delete_cross_info_config(payload, dry_run=_request_bool_arg("dry_run", False))
    return jsonify(result), 200 if result.get("status") != "error" else 400


@app.route('/intersection_config', methods=['GET'])
def list_intersection_config_api():
    full = _request_bool_arg("full", False)
    try:
        return jsonify(list_intersection_configs(full=full))
    except Exception as e:
        logger.error(f"获取路口配置列表失败: {str(e)}")
        return jsonify({
            "status": "error",
            "saved": False,
            "reason": str(e)
...
@app.route('/intersection_config/<cross_id>', methods=['GET'])
def get_intersection_config_api(cross_id):
    try:
        return jsonify(get_intersection_config(cross_id))
    except Exception as e:
        logger.error(f"获取路口配置失败: {str(e)}")
        return jsonify({
            "status": "error",
            "saved": False,
            "reason": str(e)
...
@app.route('/intersection_config/validate', methods=['POST'])
def validate_intersection_config_api():
    payload = request.get_json(silent=True)
    if not isinstance(payload, dict):
        return jsonify({
            "status": "error",
    ...
    result = validate_and_save_intersection_config(payload, dry_run=True)
    return jsonify(result), 200 if result.get("status") != "error" else 400


@app.route('/intersection_config/update', methods=['POST'])
def update_intersection_config_api():
    payload = request.get_json(silent=True)
    if not isinstance(payload, dict):
        return jsonify({
            "status": "error",
    ...
    result = validate_and_save_intersection_config(payload, dry_run=False)
    return jsonify(result), 200 if result.get("status") != "error" else 400


@app.route('/intersection_config/delete', methods=['POST'])
def delete_intersection_config_api():
    payload = request.get_json(silent=True)
    if not isinstance(payload, dict):
        return jsonify({
            "status": "error",
    ...
    dry_run = bool(payload.get("dry_run", False)) or _request_bool_arg("dry_run", False)
    result = delete_intersection_config(payload, dry_run=dry_run)
    return jsonify(result), 200 if result.get("status") != "error" else 400


@app.route('/intersection_config/<cross_id>', methods=['DELETE'])
def delete_intersection_config_by_id_api(cross_id):
    payload = request.get_json(silent=True)
    if not isinstance(payload, dict):
        payload = {}
    payload["Cross_id"] = cross_id
    payload["delete_road_info"] = _request_bool_arg("delete_road_info", payload.get("delete_road_info", True))
    payload["delete_cross_info"] = _request_bool_arg("delete_cross_info", payload.get("delete_cross_info", True))
    payload["delete_code"] = _request_bool_arg("delete_code", payload.get("delete_code", True))

    result = delete_intersection_config(payload, dry_run=_request_bool_arg("dry_run", False))
    return jsonify(result), 200 if result.get("status") != "error" else 400


@app.route('/intersection_schedule/<cross_id>', methods=['GET'])
def get_intersection_schedule_config_api(cross_id):
    day_type = request.args.get("day_type", "workday")
    try:
        return jsonify(get_schedule_config(cross_id, day_type=day_type))
    except Exception as e:
        logger.error(f"获取路口时间表配置失败: {str(e)}")
        return jsonify({
            "status": "error",
            "saved": False,
            "reason": str(e)
...
@app.route('/intersection_schedule/validate', methods=['POST'])
def validate_intersection_schedule_config_api():
    payload = request.get_json(silent=True)
    if not isinstance(payload, dict):
        return jsonify({
            "status": "error",
    ...
    result = validate_and_save_schedule_config(payload, dry_run=True)
    return jsonify(result), 200 if result.get("status") != "error" else 400


@app.route('/intersection_schedule/delete', methods=['POST'])
def delete_intersection_schedule_config_api():
    payload = request.get_json(silent=True)
    if not isinstance(payload, dict):
        return jsonify({
            "status": "error",
    ...
    dry_run = bool(payload.get("dry_run", False)) or _request_bool_arg("dry_run", False)
    result = delete_schedule_config(payload, dry_run=dry_run)
    if result.get("status") != "error" and result.get("saved"):
        for file_info in result.get("files", []):
            if file_info.get("operation") == "deleted":
                inner_time_schedule.pop(result["Cross_id"], None)
                break
    return jsonify(result), 200 if result.get("status") != "error" else 400


@app.route('/intersection_schedule/<cross_id>', methods=['DELETE'])
def delete_intersection_schedule_config_by_id_api(cross_id):
    payload = request.get_json(silent=True)
    if not isinstance(payload, dict):
        payload = {}
    payload["Cross_id"] = cross_id
    if request.args.get("day_type"):
        payload["day_type"] = request.args.get("day_type")

    result = delete_schedule_config(payload, dry_run=_request_bool_arg("dry_run", False))
    if result.get("status") != "error" and result.get("saved"):
        for file_info in result.get("files", []):
            if file_info.get("operation") == "deleted":
                inner_time_schedule.pop(result["Cross_id"], None)
...
@app.route('/intersection_schedule/update', methods=['POST'])
def update_intersection_schedule_config_api():
    global inner_time_schedule
    ...
    result = validate_and_save_schedule_config(payload, dry_run=False)
    if result.get("status") != "error":
        today_type = "workday" if is_workday(date.today()) else "weekend"
        if result.get("day_type") == today_type:
            schedule = payload.get("schedule", payload.get("Time_schedule"))
            if schedule is None:
                schedule = {
                    key: value for key, value in payload.items()
                    if str(key).isdigit() and 0 <= int(str(key)) <= 23
                }
            inner_time_schedule[result["Cross_id"]] = schedule
    return jsonify(result), 200 if result.get("status") != "error" else 400
